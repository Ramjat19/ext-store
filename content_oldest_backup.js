...
async function performEarlyScan(phase) {
  ...
  try {
    ...
    console.log(`🔎 Early scan (${phase}): Found ${inputs.length} elements`, {
    ...
    inputs.forEach((element, index) => {
      ...
      console.log(`🎯 PRE-VISIBILITY Element ${index + 1}:`, elementInfo);
    ...
    if (newElementsCount > 0) {
      ...
      if (!performanceTimestamps.firstElementDetected) {
        ...
        console.log('⏱️ TIMESTAMP: First elements detected', {
          timeToFirstElement: performanceTimestamps.firstElementDetected - performanceTimestamps.scriptStart + 'ms'
        });
      }
      
      console.log(`✅ Early scan complete. New: ${newElementsCount}, Total: ${detectedElements.size}`, {
        processingTime: processingCompleteTime - scanStartTime + 'ms'
      });
      
      // Immediately log sensitive fields
      const newElements = Array.from(detectedElements.values()).slice(-newElementsCount);
      logDetectedSensitiveFields(newElements);
      
      // Also trigger AI analysis for enhanced classification
      if (newElementsCount >= 1) { // Trigger AI even for single elements in early detection
        performanceTimestamps.aiAnalysisStart = Date.now();
        console.log('🤖 Triggering SYNCHRONOUS AI analysis for early detected elements', {
          timeSinceStart: performanceTimestamps.aiAnalysisStart - performanceTimestamps.scriptStart + 'ms'
        });
        
        await prepareForAIDetectionSync(newElements);
      }
    }
